...
generate-test-certs is a small (< 8 mb) docker image based off `alpine linux` which makes creating self signed SSL certs easier.
...
```
docker run -e SSL_SUBJECT="example.com" dmanto/generate-test-certs
----------------------------
| generate-test-certs Cert Generator |
----------------------------
...
$ docker run -v /tmp/certs:/certs \
  -e SSL_SUBJECT=test.example.com   dmanto/generate-test-certs
```
...
_Slightly more interesting example of using `dmanto/generate-test-certs` as a volume container to build and host SSL certs for the Docker Registry image_

Create the volume container for the registry from `dmanto/generate-test-certs`:

```
$ docker run \
  --name certs \
  -e SSL_SUBJECT=test.example.com \
  dmanto/generate-test-certs
----------------------------
| generate-test-certs Cert Generator |
----------------------------
...
```
$ echo "127.0.2.1       test.example.com" >> /etc/hosts
$ docker tag dmanto/generate-test-certs test.example.com:5000/omgwtfbbq
...
* `K8S_NAME` (generate-test-certs)
* `K8S_NAMESPACE` (default)
* `K8S_SAVE_CA_KEY` (false)
* `K8S_SAVE_CA_CRT` (false)
* `SILENT` (true)

An example manifest can be found at `examples/minikube/generate-test-certs.yaml`.

```
$ kubectl apply -f examples/minikube
configmap "generate-test-certs" created
job "generate-test-certs" created

$ kc get pods -a
NAME              READY     STATUS      RESTARTS   AGE
generate-test-certs-blz7m   0/1       Completed   0          2m

$ kubectl logs generate-test-certs-blz7m
secret "generate-test-certs" created
kubectl get secret generate-test-certs -o yaml
